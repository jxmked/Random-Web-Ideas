...
window.addEventListener("DOMContentLoaded", function () {
  ...
  var FRESH_ARRAY = Array.apply(null, Array(BLOCKS_DOM.length)).map(
    function () {
      return void 0;
    },
  );
  ...
  var CHECK_STRAIGHT = function (table, challenger) {
    return STRAIGHTS.filter(function (straight) {
      return (
        straight.filter(function (i) {
          return table[i] == challenger;
  ...
  var _b = useState(
      function (current_value, index, element, challenger) {
        if (challenger == "reset") return __spreadArray([], FRESH_ARRAY, true);
        var cloned = __spreadArray([], current_value, true);
        cloned[index] = challenger;
        return cloned;
      },
      __spreadArray([], FRESH_ARRAY, true),
    ),
    BLOCKS_ARRAY = _b[0],
    make_turn = _b[1],
    block_change = _b[2];
  ...
  var ENEMY = function () {
    var available_moves = BLOCKS_ARRAY()
