...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const FRESH_ARRAY: undefined[] = Array.apply(
    null,
    Array(BLOCKS_DOM.length),
  ).map(() => void 0);
  ...
  const changeText = useState((e: HTMLElement) => {
    if (PLAYER_STATUS() == "") {
      ...
      MESSAGE_DOM.innerText =
        SOME_QOUTES[Math.floor(Math.random() * SOME_QOUTES.length)];
    } else {
  ...
  const CHECK_STRAIGHT: Function = (table: string[], challenger: string) => {
    return STRAIGHTS.filter((straight: number[]) => {
      return (
        straight.filter((i: number) => table[i] == challenger).length ==
        straight.length
      );
    });
  };
  ...
  const IS_BLOCK_TAKEN: Function = (index: number) =>
    BLOCKS_ARRAY()[index] !== void 0;

  const [BLOCKS_ARRAY, make_turn, block_change] = useState(
    (current_value: string[], index: number, element: any, challenger: any) => {
      /**
       *  make_turn function will trigger this function and return cloned modified array
       * */

      if (challenger == "reset") return [...FRESH_ARRAY];
  ...
  const ENEMY: Function = () => {
    const available_moves: number[] = BLOCKS_ARRAY()
