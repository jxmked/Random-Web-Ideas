...
function useState(param:any, def?:any){
    ...
    let mainSetValue:any = (...args:any) => { // Set Value
        let res:any = null;
        if ({}.toString.call(setValue) === '[object Function]') { // Function
            // @ts-expect-error
            res = setValue(...args);
            onChange(value);
            return res;
    ...
    if(typeof param == 'string' || param instanceof String) { // String
        setValue = (arg:any) => {
            value = arg;
        };
    } else if(Number(param) === param) { // Number
        setValue = (arg:any) => {
            try {
    ...
    } else if (param.constructor === Object) { // Dictionary
        setValue = (...args:any) => {
            const [key, val] = args;
    ...
    } else if (param === true || param === false) { // Boolean
        setValue = (arg:any) => {
    ...
    } else if (param.constructor === Array) { // Array
        setValue = {
            push:(...args:any) => {
    ...
    } else if ({}.toString.call(param) === '[object Function]') { // Function
        setValue = (...args:any) => {
